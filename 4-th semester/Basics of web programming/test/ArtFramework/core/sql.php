...
//    }
    ...
    public function where($fields = [], string $operator){
        //$this->placeholders = array_merge($this->placeholders, $fields)??$fields;
        ...
        if($this->query != null) {
            if(strrpos($this->query, "WHERE")) {
                $this->query = $this->query . " AND " . $key . " " . $operator . " " . key($fields);
                return $this;
            }
            else {
